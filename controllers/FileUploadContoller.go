...
func (c *FileUploadController)FileUpload(){
	...
	imageUrl := c.Ctx.Input.Param(":imageUrl")
	fmt.Println("insideeeeeee",imageUrl)
	if len(imageUrl) != 0 {

		response, e := http.Get(imageUrl)
		if e != nil {
			fmt.Println("error is goinggg")
			log.Fatal(e)
		}
		...
		file, err := os.Create("./testUploadJson/"+"avatar"+msec+".jpg")
		if err != nil {
			fmt.Println("error ins1")
			log.Fatal(err)
		}
		// Use io.Copy to just dump the response body to the file. This supports huge files
		_, err = io.Copy(file, response.Body)
		if err != nil {
			fmt.Println("error ins2")
			log.Fatal(err)
		}
		file.Close()
		fmt.Println("Success!")
